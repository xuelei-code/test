...
ratio = 1671.8
# SDS格式的数据存储位置
# filepath='/home/xmzk/data/mseed'
# 普通文件夹格式存储位置
filepath = '/home/xmzk/data/mseed'
# outputpath是输出结果的存储位置
outputpath = '/home/zfs/intensity/PY_result'
...
# 以下开始进行pga的计算
# 读入各方向上的波形，滤波
stEs = sts.select(channel = "EIE").merge(fill_value=0, interpolation_samples=1).sort()
stNs = sts.select(channel = "EIN").merge(fill_value=0, interpolation_samples=1).sort()
...
with open(outputdir + '/intensity.txt','r') as r:
   lines=r.readlines()
...
os.system('python sendmsg_wechat.py')
os.system('python mail_file.py')
